...
public class TestEx3Controller {
	...
	public void testEx3Controller() throws Exception {
		...
		request = post("/user/add/").param("id", "1").param("name", "Louisz").param("email", "louisz6ster@gmail.com")
				.param("mobileNumber", "0912345678");
		mvc.perform(request).andExpect(content().string(equalTo("add success")));
		...
		request = post("/user/update/1").param("name", "Jessie").param("email", "louisz6ster@gmail.com")
				.param("mobileNumber", "0923456789");
		mvc.perform(request).andExpect(content().string(equalTo("update success")));
		// 5.使用指定查詢進行該資料比對，確認修改的資訊是否正確
		request = post("/user/find/1");
